...
const convertToBoolean = (value) => {
  ...
  if (typeof value === 'string') {
    ...
    if (lowered === 'sim' || lowered === 'yes' || lowered === 'true') return true;
    if (lowered === 'não' || lowered === 'nao' || lowered === 'no' || lowered === 'false') return false;
  }
  return false; 
};
  ...
  async (_, { getState, rejectWithValue }) => {
    try {
      ...
      try {
        ...
        const anamnesisData = {
          saude_geral: {
            doencas_cronicas: convertToBoolean(anamnesis.questoesGerais?.listaDoencasCronicas?.doencasCronicas),
            hipertenso: convertToBoolean(anamnesis.questoesGerais?.listaDoencasCronicas?.hipertenso),
            diabetes: convertToBoolean(anamnesis.questoesGerais?.listaDoencasCronicas?.diabetes),
            cardiopatia: convertToBoolean(anamnesis.questoesGerais?.listaDoencasCronicas?.cardiopatia),
            outras_doencas: anamnesis.questoesGerais?.listaDoencasCronicas?.outrasTexto || '',
            diagnostico_cancer: convertToBoolean(anamnesis.questoesGerais?.diagnosticoCancer),
            tipo_cancer: anamnesis.questoesGerais?.tipoCancer || '',
            uso_medicamentos: convertToBoolean(anamnesis.questoesGerais?.usoMedicamentos),
            medicamentos: anamnesis.questoesGerais?.medicamentos || '',
            possui_alergia: convertToBoolean(anamnesis.questoesGerais?.possuiAlergia),
            alergias: anamnesis.questoesGerais?.alergias || '',
            ciruturgias_dermatologicas: convertToBoolean(anamnesis.questoesGerais?.ciruturgiasDermatologicas),
            tipo_procedimento: anamnesis.questoesGerais?.tipoProcedimento || '',
            pratica_atividade_fisica: convertToBoolean(anamnesis.questoesGerais?.praticaAtividadeFisica),
          ...
          avaliacao_fototipo: {
            ...
            cor_olhos: convertToNumber(anamnesis.avaliacaoFototipo?.corOlhos, 1),
            cor_cabelo: convertToNumber(anamnesis.avaliacaoFototipo?.corCabelo, 1),
            quantidade_sardas: convertToNumber(anamnesis.avaliacaoFototipo?.quantidadeSardas, 1),
            reacao_sol: convertToNumber(anamnesis.avaliacaoFototipo?.reacaoSol, 1),
            bronzeamento: convertToNumber(anamnesis.avaliacaoFototipo?.bronzeamento, 1),
            sensibilidade_solar: convertToNumber(anamnesis.avaliacaoFototipo?.sensibilidadeSolar, 1)
          },
          historico_cancer_pele: {
            historico_familiar: convertToBoolean(anamnesis.historicoCancer?.historicoFamiliar),
            ...
            diagnostico_pessoal: convertToBoolean(anamnesis.historicoCancer?.diagnosticoPessoal) || false,
            ...
            lesoes_precancerigenas: convertToBoolean(anamnesis.historicoCancer?.lesoesPrecancerigenas) || false,
            tratamento_lesoes: convertToBoolean(anamnesis.historicoCancer?.tratamentoLesoes) || false,
          ...
          fatores_risco_protecao: {
            exposicao_solar_prolongada: convertToBoolean(anamnesis.fatoresRisco?.exposicaoSolarProlongada),
            frequencia_exposicao_solar: anamnesis.fatoresRisco?.frequenciaExposicaoSolar || "Diariamente",
            queimaduras_graves: convertToBoolean(anamnesis.fatoresRisco?.queimadurasGraves) || false,
            quantidade_queimaduras: anamnesis.fatoresRisco?.quantidadeQueimaduras || "1-2",
            uso_protetor_solar: convertToBoolean(anamnesis.fatoresRisco?.usoProtetorSolar) || false,
            fator_protecao_solar: anamnesis.fatoresRisco?.fatorProtecaoSolar || "15",
            uso_chapeu_roupa_protecao: convertToBoolean(anamnesis.fatoresRisco?.usoChapeuRoupaProtecao) || false,
            bronzeamento_artificial: convertToBoolean(anamnesis.fatoresRisco?.bronzeamentoArtificial) || false,
            checkups_dermatologicos: convertToBoolean(anamnesis.fatoresRisco?.checkupsDermatologicos) || false,
            ...
            participacao_campanhas_prevencao: convertToBoolean(anamnesis.fatoresRisco?.participacaoCampanhasPrevencao) || false
          },
          investigacao_lesoes_suspeitas: {
            mudanca_pintas_manchas: convertToBoolean(anamnesis.investigacaoLesoes?.mudancaPintasManchas),
            sintomas_lesoes: convertToBoolean(anamnesis.investigacaoLesoes?.sintomasLesoes) || false,
            tempo_alteracoes: anamnesis.investigacaoLesoes?.tempoAlteracoes || "Menos de 1 mês",
            caracteristicas_lesoes: convertToBoolean(anamnesis.investigacaoLesoes?.caracteristicasLesoes) || false,
            consulta_medica: convertToBoolean(anamnesis.investigacaoLesoes?.consultaMedica) || false,
